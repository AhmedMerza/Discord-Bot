...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (user.roles.cache.some(role => role.name === 'Owner')) return message.channel.send("You can't mute the owner buddy");
        ...
        if (user.roles.cache.some(role => role.name === 'muted')) return message.channel.send("User is already muted");
        ...
        await message.channel.send(
            `you muted ${message.mentions.users.first()}`
        );
    }
};
